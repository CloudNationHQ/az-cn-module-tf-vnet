locals {
  subnets = flatten([
    for subnet_key, subnet in try(var.vnet.subnets, {}) : {
      ...
      subnet_name                = try(subnet.subnet_name, join("-", [var.naming.subnet, subnet_key]))
      nsg_name                   = try(subnet.nsg_name, join("-", [var.naming.network_security_group, subnet_key]))
      rt_name                    = try(subnet.route_name, join("-", [var.naming.route_table, subnet_key]), {})
